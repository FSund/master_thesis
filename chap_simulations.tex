...
\begin{align*}
    ...
    &= \vvec(t) + \frac{\Fvec(t) + \Fvec(t + \Delta t)}{m} \frac{\Delta t}{2} + \mathcal{O}(\Delta t^3).
\end{align*}
...
\begin{align}
    ...
    \vvec(t + \Delta t) &= \vvec(t) + \frac{\Fvec(t) + \Fvec(t + \Delta t)}{m} \frac{\Delta t}{2}\label{eq:velocity_verlet_velocity},
\end{align}
...
\begin{align}
    \vvec(t+\Delta t) = \tilde\vvec(t + \tfrac{1}{2}\Delta t) + \frac{\Fvec(t + \Delta t)}{m}\frac{\Delta t}{2},\label{eq:verlet_velocity_with_halfstep}
\end{align}
where
\begin{align}
    \tilde\vvec(t + \tfrac{1}{2}\Delta t) = \vvec(t) + \frac{\Fvec(t)}{m}\frac{\Delta t}{2}.\label{eq:verlet_halfstep}
\end{align}
...
\orangebox{
    ...
    which gives
    \begin{align*}
        \text{error}\big(\rvec(t+2\Delta t)\big) = \text{error}(\rvec(t+\Delta t)) + \text{error}\big(\vvec(t+\Delta t)\big)\Delta t + \left(\frac{\Fvec(t + \Delta t)}{m}\right)\frac{\Delta t^2}{2}
    \end{align*}
}
