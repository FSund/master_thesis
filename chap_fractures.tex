...
\caption{Illustration of the two steps used in the diamond square algorithm for generating random surfaces.}
...
We obviously can't generate infinite surfaces, and instead we generate a surface on a finite grid of size $n\times n$. As shown in \cref{eq:diamond_step_resolution} the algorithm can go from any resolution $n_0$ to $n_p = 2^p(n_0-1) + 1$ by applying the algorithm $p$ times. On a computer we prefer allocating a matrix to store the surface 
