...
\begin{figure}
    \centering
    \includesvg[width=0.5\textwidth, svgpath = images/diamond_square/]{increase_resolution_solarized}
    \caption{
        Caption.
    }
\end{figure}

As mentioned above, the method called \emph{successive random additions} is a modification of the regular midpoint displacement method first described by Richard F. Voss\cite{voss1985random}, where we add random numbers to \emph{all} points in each iteration, compared to just the \emph{new} points in the regular midpoint displacement method. This modification means that we can replace the factor $(1/2)$ in \cref{eq:midpoint_sigma_first,eq:midpoint_sigma_general} with a general factor $r$\todo{why? (something about distance between points)}, and we get
\begin{align*}
...
The central part of the algorithm consists of two steps often called the \emph{diamond-step} and the \emph{square-step}. For simplicity we start with a simple case of an infinite grid of evenly spaced points, all with known $z$-values. The two steps are as follows:
\begin{description}
    \item[The \emph{square-step}:] Generate the $z$-value in the center of all squares of 2-by-2 points by averaging the $z$-values of the four corners of each square, as indicated by the red dots in \cref{fig:initial_square_step}. Then add a random Gaussian number with mean $\mu = 0$ and variance $\sigma_i^2 = \sigma_{i-1}^2r^{2H}$ to all new and old points, where $\sigma_{i-1}^2$ is the variance used in the last step of the algorithm.
    \label{enum:test}
    
    \item[The \emph{diamond-step}:] Generate the $z$-values in the middle of the four edges of each square of 2-by-2 points by averaging the $z$-values of the points above, below, to the right of, and to the left of the new points, as indicated by the red dots in \cref{fig:diamond_step}. We then add a random Gaussian number with mean $\mu = 0$ and variance $\sigma_{i+1}^2 = \sigma_i^2r^{2H}$, to all new and old points. 
\end{description}
\hl{$H$ is the wanted Hurst exponent, and $r < 1$ is a parameter that controls the \emph{lacunarity} of the fractal.}\todo{define lacunarity and $r$}

\hl{Using first the square-step and then the diamond-step we can double the resolution}
...
\setlength{\mycaptionwidth}{0.4\textwidth}
