...
Voss has geneneralized the the method of successive random additions to higher dimensions\cite{voss1985random}, and this generalized form is the algorithm we use when generating fractures. We generate terrains in the form of heightmaps, meaning a 2-dimensional grid of points ({$x,y$}), with a value for the height in each point \hl{($z(x,y) = z_{x,y}$)} which is generated by the algorithm.
...
\begin{minipage}[c]{\myfigwidth}
    \includesvg[width=\textwidth, svgpath = images/diamond_square/]{simple_square_step_solarized}
\end{minipage}
...
\begin{minipage}[c]{\myfigwidth}
    \includesvg[width=\textwidth, svgpath = images/diamond_square/]{simple_diamond_step_solarized}
\end{minipage}
\begin{minipage}[c]{\mycaptionwidth}
...
\begin{figure}
    \centering
    \includesvg[width=0.7\textwidth, svgpath = images/diamond_square/]{increase_resolution_solarized}
    \caption{
...
\subsubsection{Implementing successive random additions on a finite grid in 2 dimensions}
We obviously can't generate infinite terrains, instead we generate a terrain on a finite grid of size $n\times n$. On a computer we would like to allocate a matrix for storing each point in the final terrain before using the algorithm, which means we have to know the size beforehand. As shown in \cref{eq:diamond_step_resolution} the algorithm can go from any resolution $n_0$ to $n_i = 2^i(n_0-1) + 1$. In our implementation of the algorithm we start from a grid with the $z$-value in each corner defined (a resolution of $2\times 2$), which means our final resolution is $n_i = 2^i + 1$ after $i$ iterations, and that we should allocate a matrix of size $(2^i + 1) \times (2^i + 1)$ in the initialization of the program.

When generating points near the edges of the grid we see that we can run into some problems when using the diamond-step, since this step calculates the average of the points above, below, to the right of, and to the left of the point we are generating. When close to the edges some of these points might end up being outside the grid. We can solve this by either ignoring the points outside the grid when calculating the averages, or wrapping around the edges using periodic boundary conditions to get a point on the opposite side of the grid. We want our implementation of the algorithm to be able to produce both periodic and non-periodic terrains, so we implement both solutions.

This implementation means that when for example generating the point in the center of the left edge of the grid, $z(n/2, 0)$, we need to calculate the average over the corners of the tilded square as follows
\begin{align*}
    z\left(\tfrac{n}{2}, 0\right) = 
    \frac{1}{4}
    \left(
        z\left(\tfrac{n}{2},-\tfrac{n}{2}\right) + z\left(\tfrac{n}{2},\tfrac{n}{2}\right) + z(0,0) + z\left(n,\tfrac{n}{2}\right)
    \right).
\end{align*}
If we are generating a non-periodic terrain we ignore the point $z(n/2, -n/2)$ which is outside the grid, and get
\begin{align*}
    z\left(\tfrac{n}{2}, 0\right) = 
    \frac{1}{3}
    \left(
        z\left(\tfrac{n}{2},\tfrac{n}{2}\right) + z(0,0) + z\left(n,\tfrac{n}{2}\right)
    \right),
\end{align*}
but if we are generating a periodic terrain we translate the point $z(n/2, -n/2)$ inside the grid to $z(n/2, n/2)$ using periodic boundary conditions, and get
\begin{align*}
    z\left(\tfrac{n}{2}, 0\right) = 
    \frac{1}{4}
    \left(
    ...
    \right).
\end{align*}
\hl{We see that in this case we actually end up using $z(n/2, n/2)$ twice}

To ensure that the generated terrains are periodic (when generating a periodic terrain) we let the right and bottom edge be equivalent to the left and top edge, respectively (this means that the four corners are equivalent). This effectively means that the resolution of the periodic terrains we generate is $2^n$ instead of $2^n+1$

% \hl{We implement the periodic boundary conditions by letting the right and bottom edges of the grid always have the same $z$-values as the left and top edges. This}

This leaves us with the following algorithm for generating a terrain which approximates a 2-dimensional fractional Brownian motion with Hurst exponent $H$
\begin{enumerate}
