...
We obviously can't generate infinite terrains, and instead we generate a terrain on a finite grid of size $n\times n$. As shown in \cref{eq:diamond_step_resolution} the algorithm can go from any resolution $n_0$ to $n_p = 2^p(n_0-1) + 1$ by applying the algorithm $p$ times.
...
On a computer we would like to allocate a matrix for storing each point in the final terrain before using the algorithm, which means we have to know the size beforehand.
