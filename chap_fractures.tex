...
\end{description}
We now see that by first applying the square-step and then applying the diamond-step, we double the resolution of the grid, and increase the number of points from $(2^n+1)^2$ to $(2^{n+1}+1)^2$\todo{show this, or remove it?}. An illustration where we use the algorithm on several points can be seen in \cref{fig:diamond_square_applied}.
