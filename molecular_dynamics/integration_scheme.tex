...
\end{align}
where $\Delta t$ is the timestep\todo{define timestep}, and $\avec(t)$ is the velocity at time $t$. This form of the scheme has a truncation error in the position for one timestep of the order $\mathcal{O}(\Delta t^4)$ (see \cref{appendix:verlet_taylor}).
...
The velocity Verlet algorithm is usually rewritten in the following way, to optimize the implementation on a computer. The new velocities $\vvec(t+\Delta t)$ can be written as
\begin{align}
