...
\todo[inline]{Timestep set according to vibrational frequency of water?}
...
$V_{ijk}^{(3)}$ is the three-body term, which take into account bending and stretching of covalent bonds. This term depends on the distances between atom $i$, $j$ and $k$, and also the angle $\theta_{ijk}$ between the atoms. The term has the form\todo{split into $f(\rvec_{ij}, \rvec_{ik})$ and $p(\theta_{ijk}, \theta_0)$ as in vashista 1990?}
\begin{align*}
...
\todo[inline]{More about how parameters were decided, see Mathilde's master, and [25] Wang et al in it}
