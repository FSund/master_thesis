...
The interatomic potential\cite{vashishta1990interaction} \todoco{not exactly same as vashishta, see nanobubble supplements} we use for for both silica and water consists of a two-body and a three-body part, and has the form
\begin{align*}
...
The potential is further optimized by linearising it, meaning that the potential is calculated for example for different distances $r_{ij}$ and angles $\theta_{ijk}$, and then we can look up the values for the potential directly, without having to evaluate the potential.\todobo{refine this}
