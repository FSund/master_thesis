...
For each subsurface located at $(i,j)$ we find a point $(k_m, l_m)$ in the subsurface, at
\begin{align*}
%     (k^*, l^*) = (i,j) + (n-m, n-m) %
%     (k_0, l_0) = (i,j) + (n-m, n-m) %
    (k_m, l_m) = (i,j) + (n-m, n-m), %
...
% \begin{description}%
%     \item[$\bm{\theta = 0}$:] \\ the point $(k_m, l_m)$ is in the upper right corner of the subsurface, at $(i+n,i+n)$
%     \item[$\bm{\theta = 1/2}$:] the point $(k_m, l_m)$ is in the center, at $(i+n/2, i+n/2)$
...
It can be shown that this generalized variance goes as \hl{SOURCE}
\begin{align*}
...
A modification of the method mentioned in \cite{carbone2007algorithm} is replacing $\bar f_n(i,j)$ in \cref{eq:dma_variance} with\todo{not correct, fix this}
\begin{align*}
    \tilde f_n^*(i,j) = (1-\alpha) f_n(i,j) + \alpha \tilde f_n(i-1,j-1),
\end{align*}
where $\alpha = n^2/(n+1)^2$, and $\tilde f_n(i,j)$ has the same form as before (see \cref{eq:carbone_average}). This modification \hl{can give/gives} better results \hl{source?}, so we have  implemented both the regular, and the modified method.\todo{not true, only have $\tilde f^*$}
